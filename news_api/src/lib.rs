...
impl NewsAPI {
    ...
    pub async fn fetch_async(&self) -> Result<NewsAPIResponse, NewsApiError> {
        ...
        let request = client
            ...
            .header("User-Agent", "my-news-app/1.0")
            .build()
...
fn map_response_err(code: Option<String>) -> NewsApiError {
    if let Some(code) = code {
        ...
        match code.as_str() {
            ...
            "apiKeyExhausted"=>NewsApiError::BadRequest("Api key is exhausted"),
            "apiKeyInvalid"=>NewsApiError::BadRequest("Api key is invalid"),
            "apiKeyMissing"=>NewsApiError::BadRequest("Api key is missing"),
            "parameterInvalid"=>NewsApiError::BadRequest("Parameter invalid"),
            "parametersMissing"=>NewsApiError::BadRequest("Parameter missing"),
            "rateLimited"=>NewsApiError::BadRequest("Rate limited"),
            "sourcesTooMany"=>NewsApiError::BadRequest("Sources too many"),
            "sourceDoesNotExist"=>NewsApiError::BadRequest("Source does not exist"),
            "unexpectedError"=>NewsApiError::BadRequest("Unexpected error"),
